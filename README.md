...
- **Repository:** `MilaArtyNew/discord-alert`
- **Primary stack:** Python, systemd, Railway
- **Entrypoints and scripts:**
  - `bot.py`
- **Notable dependencies:** `discord.py-self`, `python-dotenv`, `python-telegram-bot`

## Configuration

Configure the service with environment variables. Do not commit real secrets to the repository.

- `CHANNEL_IDS` — required or optional runtime configuration. See deployment environment for the actual value.
- `DISCORD_TOKEN` — required or optional runtime configuration. See deployment environment for the actual value.
- `KEYWORDS` — required or optional runtime configuration. See deployment environment for the actual value.
...
```bash
git clone https://github.com/MilaArtyNew/discord-alert
cd discord-alert
python3 -m venv .venv
